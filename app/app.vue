...
// We use a CDN to avoid complex build configuration issues with workers in Nuxt
onMounted(() => {
...
const extractMrzFromText = (text: string): string[] => {
  ...
  const mrzLines = lines.filter(line => {
    ...
    const isValidLength = cleanLine.length >= 28 && cleanLine.length <= 90 // Allow some margin
...
const processMrz = async () => {
  ...
  try {
    ...
    // Try to take the last 2 or 3 lines if there are many
    // MRZ is usually at the bottom
    let validParse = null
    let errorMsg = ''
    
    // Try different combinations of lines (last 2 or last 3)
    const potentialBlocks = []
    if (mrzLines.length >= 3) {
      potentialBlocks.push(mrzLines.slice(-3)) // Try 3 lines (TD1)
    }
    if (mrzLines.length >= 2) {
      potentialBlocks.push(mrzLines.slice(-2)) // Try 2 lines (TD2/TD3)
    }
    
    // Also try the whole set if small
    if (mrzLines.length <= 3 && !potentialBlocks.length) {
       potentialBlocks.push(mrzLines)
    }
    ...
    if (!validParse && mrzLines.length > 0) {
        // Just try the last found lines
        try {
            validParse = parse(mrzLines)
        } catch(e) {
            errorMsg = (e as Error).message
        }
    }
    ...
    if (validParse) {
      parsedResult.value = {
        documentType: validParse.fields.documentType || 'Unknown',
        documentNumber: validParse.fields.documentNumber || 'Unknown',
        firstName: validParse.fields.firstName || 'Unknown',
        lastName: validParse.fields.lastName || 'Unknown',
        nationality: validParse.fields.nationality || 'Unknown',
        birthDate: validParse.fields.birthDate || 'Unknown',
        sex: validParse.fields.sex || 'Unknown',
        expirationDate: validParse.fields.expirationDate || 'Unknown',
        issuingCountry: validParse.fields.issuingState || 'Unknown',
        valid: validParse.valid,
        details: validParse.details || []
      }
    } else {
      error.value = `MRZ detected but parsing failed: ${errorMsg}`
    }
    ...
    <div v-if="parsedResult" class="results">
      ...
      <div class="result-grid">
        ...
        <div class="result-item">
          ...
          <span>{{ parsedResult.issuingCountry }}</span>
        </div>
        <div class="result-item">
        ...
        <div class="result-item">
          <label>Sex</label>
          <span>{{ parsedResult.sex === 'M' ? 'Male' : parsedResult.sex === 'F' ? 'Female' : parsedResult.sex }}</span>
        </div>
      </div>
    </div>
